...
public class ItemTarDetController {
    ...
    public static final String CREATE_FITEM_TAR_DET_TABLE = "CREATE  TABLE IF NOT EXISTS " + TABLE_FITEM_TAR_DET +
            ...
            FITEMTARDET_TXNDATE + " TEXT, " + FITEMTARDET_ITEMCODE + " TEXT, " + FITEMTARDET_VOLUME + " TEXT); ";
        ...
        try {
            ...
            for (ItemTarDet itemTarDet : list) {
                ...
                Cursor cursor = dB.rawQuery("SELECT * FROM " + TABLE_FITEM_TAR_DET + " WHERE " +
                        FITEMTARDET_REPNO + "='" + itemTarDet.getRefNo() + "'  and " + FITEMTARDET_ITEMCODE +
                ...
                values.put(FITEMTARDET_ITEMCODE, itemTarDet.getItemcode());
                values.put(FITEMTARDET_VOLUME, itemTarDet.getVolume());

                if (cursor.getCount() > 0) {
                    dB.update(TABLE_FITEM_TAR_DET, values, FITEMTARDET_REPNO + " = '" +itemTarDet.getRefNo() + "' and " + FITEMTARDET_ITEMCODE + " = '" + itemTarDet.getItemcode()+"'", null);
                    Log.v(TAG, "Updated");
                } else {
